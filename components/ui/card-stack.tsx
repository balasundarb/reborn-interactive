...
export type CardStackProps<T extends CardStackItem> = {
  ...
  /** Card sizing - CHANGED TO PORTRAIT */
  cardWidth?: number;
  cardHeight?: number;
...
export function CardStack<T extends CardStackItem>({
  ...
  // CHANGED: Portrait dimensions (320x520 instead of 520x320)
  cardWidth = 320,
  cardHeight = 520,

  // ADJUSTED: Less overlap for portrait cards
  overlap = 0.35,
  // ADJUSTED: Wider spread for better visual balance
  spreadDeg = 60,

  perspectivePx = 1100,
  depthPx = 140,
  // ADJUSTED: Less tilt for portrait cards
  tiltXDeg = 8,
  ...
  // ADJUSTED: Less spacing for portrait cards
  ...
  return (
    ...
    >
      {/* Stage - INCREASED HEIGHT FOR PORTRAIT CARDS */}
      <div
        className="relative w-full"
        style={{ height: Math.max(580, cardHeight + 100) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
        {/* background wash / spotlight - ADJUSTED FOR PORTRAIT */}
        <div
          className="pointer-events-none absolute inset-x-0 top-6 mx-auto h-60 w-[50%] rounded-full bg-black/5 blur-3xl dark:bg-white/5"
          aria-hidden="true"
        />
        <div
          className="pointer-events-none absolute inset-x-0 bottom-0 mx-auto h-40 w-[60%] rounded-full bg-black/10 blur-3xl dark:bg-black/30"
          aria-hidden="true"
        />
        ...
        >
          <AnimatePresence initial={false}>
            {items.map((item, i) => {
              ...
              // ADJUSTED: More vertical spacing for taller cards
              const y = abs * 15;
              const z = -abs * depthPx;
                ...
                >
                  ...
                  >
                    {renderCard ? (
                      ...
                      <DefaultPortraitCard item={item} active={isActive} />
                    )}
                  </div>
                </motion.div>
              );
...
// NEW: Portrait card layout
function DefaultPortraitCard({ item }: { item: CardStackItem; active: boolean }) {
  return (
    <div className="relative h-full w-full flex flex-col">
      {/* image - TOP SECTION FOR PORTRAIT */}
      ...
      {/* content - BOTTOM SECTION FOR PORTRAIT */}
      <div className="bg-gradient-to-b from-background to-background/95 p-6">
        {/* tag */}
        {item.tag && (
          <div className="mb-3">
