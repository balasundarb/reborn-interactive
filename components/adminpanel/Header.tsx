...
export default function Header() {
  ...
  // ── Get locale from URL params so hrefs are always correct ──────────────────
  const params = useParams();
  const locale = (params?.locale as string) ?? "en";
  const base = `/${locale}/adminpanel`;

  // ── Nav items (locale-aware) ─────────────────────────────────────────────────
  const navItems = [
    { name: "Dashboard", href: `${base}`,        icon: LayoutDashboard },
    { name: "Users",     href: `${base}/users`,   icon: Grid },
    { name: "Newsletter",href: `${base}/newsletter`, icon: Shield },
  ];
  ...
  return (
    <>
      <style jsx global>{`
        .active-glow { filter: drop-shadow(0 0 5px ${ACCENT}); }
        .nav-transition { transition: all 0.5s cubic-bezier(0.4, 0, 0.2, 1); }
      `}</style>
      ...
      >
        ...
        >
          {/* ── Logo ── */}
          <Link href={`/${locale}`} className="flex-shrink-0 px-2">
          ...
          <nav className="hidden md:flex items-center gap-1 flex-1 justify-center">
            {navItems.map((item) => {
              // Match active state: exact for dashboard, prefix for sub-pages
            ...
            <div className="relative" ref={menuRef}>
              ...
              >
                <div className="relative">
                  <div
                    className={`nav-transition rounded-full bg-gradient-to-tr from-[#8e1a1a] to-[#d63031] flex items-center justify-center font-black text-white
                ...
                <div
                  className={`nav-transition overflow-hidden hidden lg:block text-left
                    ${shouldHideText ? "max-w-0 opacity-0" : "max-w-[150px] opacity-100"}
                  `}
                >
                  <p className="text-[11px] font-bold text-zinc-100 leading-none truncate">
              ...
              {isMenuOpen && (
                <div
                  className={`absolute right-0 w-52 rounded-2xl border border-white/10 bg-[#09090b]/95 backdrop-blur-xl p-2 shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-in fade-in slide-in-from-top-4 z-[60]
                    ${scrolled ? "top-[120%]" : "top-full mt-4"}
                  `}
                >
                  <div className="px-3 py-2 mb-1 border-b border-white/5 lg:hidden">
                    <p className="text-xs font-bold text-white truncate">{user.name || "Admin"}</p>
                    <p className="text-[10px] text-zinc-500 font-mono mt-0.5 truncate">{user.email}</p>
                  </div>
                  <button
          ...
          {/* ── Mobile drawer ── */}
          {isMobileMenuOpen && (
            <div className="md:hidden absolute top-full left-0 right-0 mt-2 mx-4 p-4 rounded-3xl space-y-2 animate-in slide-in-from-top-5 z-50 bg-black/90 backdrop-blur-xl border border-white/10">
              {navItems.map((item) => {
                const isActive = item.href === base
                  ? pathname === base
                  : pathname.startsWith(item.href);
                return (
                  <Link
                    ...
                    className={`flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${
                      isActive ? "bg-red-500/10 text-white" : "text-zinc-300 hover:bg-white/5"
                    }`}
                  >
