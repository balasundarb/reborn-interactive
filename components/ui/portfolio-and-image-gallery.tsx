...
// Moved GSAP registration inside a useEffect to prevent hydration issues
...
function useResponsiveValue(baseValue: number, mobileValue: number) {
  ...
  // Return baseValue during SSR and initial render to prevent hydration mismatch
  return isMounted ? value : baseValue;
}
...
>(
  (
    ...
    // Helper function to format numbers consistently between server and client
    const formatNumber = (num: number, precision: number = 3) => {
      // During SSR, use consistent precision
      if (!isClient) {
        return num.toFixed(precision);
      }
      // On client, let the browser handle it naturally
      return num;
    };
        ...
        >
          ...
          >
            {childrenNodes.map((child, index) => {
              ...
              const formattedRotation = formatNumber(rotationAngle + 90);
              ...
              return (
                <li
                  ...
                  style={{
                    // Use kebab-case for CSS properties to match server rendering
                    zIndex: isHovered ? 100 : 10, // React will convert this to kebab-case
                    transform: `translate(-50%, -50%) translate3d(${formattedX}px, ${formattedY}px, 0) rotate(${formattedRotation}deg)`,
                  }}
                >
                  <div
                    role='button'
